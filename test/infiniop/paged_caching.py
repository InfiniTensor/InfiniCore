...
#     (5, 512, 40, 128, 16),
#     (16, 1024, 8, 64, 32),
#     (3, 20, 1, 80, 8), # Test with small values
# ]
_TEST_CASES_ = [
    (1, 128, 8, 128, 16),
    (5, 512, 40, 128, 16),
    (16, 1024, 8, 64, 32),
    (10, 1024, 40, 64, 32),
    # (1, 1, 4, 1, 2), # Test with small values
]
...
def test(
    ...
    context_lens_torch = torch.randint(1, max_seq_len + 1, (num_seqs,), dtype=torch.int32)
    ...
    slot_mapping_list = []
    # We need to ensure allocated blocks do not overlap. Let's simulate a simple block allocator.
    allocated_slots = set()
    current_slot = 0
    ...
    assert current_slot <= num_blocks * block_size, "Not enough blocks in the cache pool for this test case"
    ...
    # # Create input tensors based on the calculated total tokens (ntok)
    # k_torch = torch.ones((ntok, num_kv_heads, head_size), dtype=torch.float16)
    # v_torch = torch.ones((ntok, num_kv_heads, head_size), dtype=torch.float16)
    # k = TestTensor.from_torch(k_torch, dtype, device)
    # v = TestTensor.from_torch(v_torch, dtype, device)
    # slot_mapping = TestTensor.from_torch(slot_mapping_torch, InfiniDtype.I32, device)

    # # The cache pools are the "output" tensors for this operator
    # k_cache_pool_torch = torch.zeros((num_blocks, num_kv_heads, block_size, head_size), dtype=torch.float16)
    # v_cache_pool_torch = torch.zeros((num_blocks, num_kv_heads, block_size, head_size), dtype=torch.float16)
    # k_cache_pool = TestTensor.from_torch(k_cache_pool_torch, dtype, device)
    # v_cache_pool = TestTensor.from_torch(v_cache_pool_torch, dtype, device)
