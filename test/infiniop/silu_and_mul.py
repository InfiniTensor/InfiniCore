...
def test(
    ...
    if not (a.torch_tensor().is_contiguous() and 
            c.torch_tensor().is_contiguous() and 
            ans.torch_tensor().is_contiguous()):
    ...
    # Profiling workflow
    if PROFILE:
        profile_operation("PyTorch", lambda: torch_silu_and_mul_reference(), device, NUM_PRERUN, NUM_ITERATIONS)
        profile_operation("    lib", lambda: lib_op(), device, NUM_PRERUN, NUM_ITERATIONS)
