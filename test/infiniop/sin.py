...
_TOLERANCE_MAP = {
    ...
    InfiniDtype.F64: {"atol": 1e-11, "rtol": 1e-11},
}
...
def test(
    ...
    if inplace == Inplace.INPLACE:
        ...
        # 确保输出张量的形状与输入张量的形状匹配
        output_shape = shape if output_stride is None else shape
        output = TestTensor(output_shape, output_stride, dtype, device, mode="ones")
    ...
    sin(output.torch_tensor(), input_tensor.torch_tensor())
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: sin(output.torch_tensor(), input_tensor.torch_tensor()), device, NUM_PRERUN, NUM_ITERATIONS)
