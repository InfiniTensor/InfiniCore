...
def test_op(handle, device, dtype=InfiniDtype.BF16, sync=None):
    print(
        f"Testing DSV4 sglang_compress_c128_v2_prefill on {InfiniDeviceNames[device]}"
    )
    ...
    ape_f32 = torch.randn(ratio, head_dim, dtype=torch.float32, device="cuda")
    kv_input_t = kv_input_f32.to(torch.bfloat16).contiguous()
    ape_t = ape_f32.to(torch.bfloat16).contiguous()
    kv_buffer_t = torch.zeros(
        2, ratio, head_dim * 2, dtype=torch.bfloat16, device="cuda"
    )
    ...
    check_error(
        LIBINFINIOP.infiniopCreateDsv4SglangCompressC128V2PrefillDescriptor(
            handle,
            ctypes.byref(desc),
    ...
    workspace, workspace_size = _workspace(
        desc,
        LIBINFINIOP.infiniopGetDsv4SglangCompressC128V2PrefillWorkspaceSize,
        device,
    )
    check_error(
        LIBINFINIOP.infiniopDsv4SglangCompressC128V2Prefill(
            desc,
            workspace.data(),
            workspace_size,
            kv_buffer.data(),
    ...
    expected = _ref_compress_c128(kv_input_f32, ape_f32, 127, head_dim)
    assert torch.allclose(
        out.actual_tensor()[0].float(), expected, atol=5e-2, rtol=5e-2
    )
    check_error(
        LIBINFINIOP.infiniopDestroyDsv4SglangCompressC128V2PrefillDescriptor(desc)
    )
...
if __name__ == "__main__":
    ...
    print("\033[92mDSV4 sglang_compress_c128_v2_prefill Test passed!\033[0m")
