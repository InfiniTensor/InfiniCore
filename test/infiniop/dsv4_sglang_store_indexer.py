...
def test_op(handle, device, num_tokens, head_dim, dtype=InfiniDtype.BF16, sync=None):
    print(
        f"Testing DSV4 sglang_store_indexer on {InfiniDeviceNames[device]} tokens:{num_tokens} head_dim:{head_dim}"
    )
    if not hasattr(torch, "float8_e4m3fn"):
        print("torch float8_e4m3fn unavailable, skipping dequant assertion")
    page_size = 64
    page_bytes = (head_dim + 4) * page_size
    ...
    check_error(
        LIBINFINIOP.infiniopCreateDsv4SglangStoreIndexerDescriptor(
            handle,
            ctypes.byref(desc),
            x.descriptor,
            cache.descriptor,
    ...
    workspace, workspace_size = _workspace(
        desc, LIBINFINIOP.infiniopGetDsv4SglangStoreIndexerWorkspaceSize, device
    )
    check_error(
        LIBINFINIOP.infiniopDsv4SglangStoreIndexer(
            desc,
            workspace.data(),
            workspace_size,
            x.data(),
    ...
    if True and hasattr(torch, "float8_e4m3fn"):
    ...
    check_error(LIBINFINIOP.infiniopDestroyDsv4SglangStoreIndexerDescriptor(desc))
...
if __name__ == "__main__":
    ...
    print("\033[92mDSV4 sglang_store_indexer Test passed!\033[0m")
