...
def test_op(handle, device, dtype=InfiniDtype.BF16, sync=None):
    print(f"Testing DSV4 sglang_compress_c4_v2_prefill on {InfiniDeviceNames[device]}")
    head_dim, ratio, seq_len = 512, 4, 8
    req_pool_indices, req_to_token, full_to_swa, seq_lens, extend_lens = _paged_inputs(
        1, seq_len
    )
    kv_input_f32 = torch.randn(
        seq_len, head_dim * 4, dtype=torch.float32, device="cuda"
    )
    ape_f32 = torch.randn(ratio * 2, head_dim, dtype=torch.float32, device="cuda")
    kv_input_t = kv_input_f32.to(torch.bfloat16).contiguous()
    ape_t = ape_f32.to(torch.bfloat16).contiguous()
    kv_buffer_t = torch.zeros(
        4, ratio, head_dim * 4, dtype=torch.bfloat16, device="cuda"
    )
    plan_c_t = torch.empty(seq_len, 16, dtype=torch.uint8, device="cuda")
    plan_w_t = torch.empty(seq_len, 8, dtype=torch.uint8, device="cuda")
    pin_buffer = torch.empty(seq_len * (16 + 8), dtype=torch.uint8, pin_memory=True)
    lens = _func("sglang_compress_v2_plan_prefill_out")(
        req_pool_indices,
        req_to_token,
        full_to_swa,
        seq_lens,
    ...
    assert (num_c, num_w) == (2, 4)
    out_t = torch.empty(num_c, head_dim, dtype=torch.bfloat16, device="cuda")
    ...
    check_error(
        LIBINFINIOP.infiniopCreateDsv4SglangCompressC4V2PrefillDescriptor(
            handle,
            ctypes.byref(desc),
    ...
    workspace, workspace_size = _workspace(
        desc, LIBINFINIOP.infiniopGetDsv4SglangCompressC4V2PrefillWorkspaceSize, device
    )
    check_error(
        LIBINFINIOP.infiniopDsv4SglangCompressC4V2Prefill(
            desc,
            workspace.data(),
            workspace_size,
            kv_buffer.data(),
    ...
    expected = torch.stack(
        [
            _ref_compress_c4(kv_input_f32, ape_f32, 3, head_dim),
            _ref_compress_c4(kv_input_f32, ape_f32, 7, head_dim),
        ]
    )
    assert torch.allclose(out.actual_tensor().float(), expected, atol=5e-2, rtol=5e-2)
    check_error(
        LIBINFINIOP.infiniopDestroyDsv4SglangCompressC4V2PrefillDescriptor(desc)
    )
...
if __name__ == "__main__":
    ...
    for device in get_test_devices(args):
        test_operator(device, test_op, [()], [InfiniDtype.BF16])
    print("\033[92mDSV4 sglang_compress_c4_v2_prefill Test passed!\033[0m")
