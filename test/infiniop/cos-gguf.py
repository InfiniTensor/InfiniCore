...
def test(
    ...
    print(
        f"Testing Cos on {InfiniDeviceNames[device]} with shape:{shape} x_stride:{x_stride} "
        f"c_stride:{c_stride} dtype:{InfiniDtypeNames[dtype]} inplace:{inplace}"
    )

    cos(c.torch_tensor(), x.torch_tensor())
    if sync is not None:
        sync()
    ...
    check_error(LIBINFINIOP.infiniopGetCosWorkspaceSize(descriptor, ctypes.byref(workspace_size)))
    ...
    if PROFILE:
        profile_operation("PyTorch", lambda: cos(c.torch_tensor(), x.torch_tensor()), device, NUM_PRERUN, NUM_ITERATIONS)
        profile_operation("    lib", lib_cos, device, NUM_PRERUN, NUM_ITERATIONS)
...
if __name__ == "__main__":
    ...
    # gguf 文件路径示例，按实际情况修改
    _TEST_CASES = {
        InfiniDtype.F16: load_test_cases_from_gguf("T1-1-1/cos/cos_bf16.gguf"),
        InfiniDtype.F32: load_test_cases_from_gguf("T1-1-1/cos/cos_f32.gguf"),
        InfiniDtype.BF16: load_test_cases_from_gguf("T1-1-1/cos/cos_bf16.gguf"),
    }
