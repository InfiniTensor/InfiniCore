...
def test(
    ...
    for r in range(num_rounds):
        ...
        slot_mapping = TestTensor.from_torch(slots_torch, InfiniDtype.I32, device)
        ...
        def torch_caching():
            ...
            return ref_paged_caching(
                ...
                k_pool_ref, v_pool_ref, 
                slots_torch, block_size
            )
        ...
        assert torch.allclose(k_cache_pool.actual_tensor(), k_pool_ref, atol=atol, rtol=rtol)
        assert torch.allclose(v_cache_pool.actual_tensor(), v_pool_ref, atol=atol, rtol=rtol)

        # 5. Profiling
        if PROFILE:
            profile_operation(f"Torch_R{r}", lambda: torch_caching(), device, NUM_PRERUN, NUM_ITERATIONS)
            profile_operation(f"  Lib_R{r}", lambda: lib_caching(), device, NUM_PRERUN, NUM_ITERATIONS)
