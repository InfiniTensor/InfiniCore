...
def softmax(x):
    type = x.dtype
    return torch.nn.functional.softmax(x.to(torch.float32), dim=-1).to(type)


def bi_attention(q, k, v, k_cache, v_cache, pos):
    type = q.dtype
    ...
    # Scaled dot-product bi_attention
    attn_scores = (
    ...
    attn_weights = softmax(attn_scores).reshape(
...
def test(
    ...
    print(
        f"Testing BiAttention on {InfiniDeviceNames[device]} with n_q_head:{n_q_head} n_kv_head:{n_kv_head} seq_len:{seq_len} head_dim:{head_dim} pos:{pos} "
    ...
    check_error(
        LIBINFINIOP.infiniopCreateBiAttentionDescriptor(
            handle,
    ...
    check_error(
        LIBINFINIOP.infiniopGetBiAttentionWorkspaceSize(
            descriptor, ctypes.byref(workspace_size)
        )
    )
    workspace = TestWorkspace(workspace_size.value, out.device)

    def lib_bi_attention():
        check_error(
            LIBINFINIOP.infiniopBiAttention(
                descriptor,
                workspace.data(),
    ...
    if PROFILE:
        # fmt: off
        profile_operation("PyTorch", lambda: torch_bi_attention(), device, NUM_PRERUN, NUM_ITERATIONS)
        profile_operation("    lib", lambda: lib_bi_attention(), device, NUM_PRERUN, NUM_ITERATIONS)
        # fmt: on
    check_error(LIBINFINIOP.infiniopDestroyBiAttentionDescriptor(descriptor))
