...
def parse_test_cases():
    ...
    for num_seqs, max_seq_len, num_kv_heads, head_size, block_size in _TEST_CASES_DATA:
        ...
        for dtype in _TENSOR_DTYPES:
            ...
            slot_mapping_spec = TensorSpec.from_tensor(slot_mapping_shape,init_mode=TensorInitializer.MANUAL, set_tensor=slot_mapping,dtype=infinicore.int32)
            
            # In-place operation: modifies k_cache (index 2) and v_cache (index 3)
            test_cases.append(
                TestCase(
                    inputs=[k_spec, v_spec, k_cache_spec, v_cache_spec, slot_mapping_spec],
                    kwargs=None,
                    output_spec=None,
