...
def where_op(
    ...
    """NumPy reference implementation of where operation"""
...
class WhereTestCase(InfiniopTestCase):
    ...
    def write_test(self, test_writer: "InfiniopTestWriter"):
        ...
        ans = where_op(self.condition, self.a, self.b)
        test_writer.add_tensor(
...
if __name__ == "__main__":
    test_writer = InfiniopTestWriter("where_op.gguf")
    test_cases = []
    ...
    # Support all legal data types
    _TENSOR_DTYPES_ = [np.bool_, np.int8, np.int16, np.int32, np.int64,
                       np.float32, np.float16, np.float64,
                       np.uint8, np.uint16, np.uint32, np.uint64,
                       np.bfloat16]
    # _TENSOR_DTYPES_ = [np.float32, np.float16, np.float64, np.bfloat16]
    
    for dtype in _TENSOR_DTYPES_:
        for shape, stride_a, stride_b, stride_c, stride_condition in _TEST_CASES_:
