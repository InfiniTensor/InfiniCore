...
class InfiniCoreModule:
    ...
    def parameters(self, recurse: bool = True) -> Iterator[Tuple[str, torch.nn.Parameter]]:
        ...
        if recurse:
            for name, module in self._modules.items():
                ...
                if isinstance(module, InfiniCoreModule):
                    for subname, subparam in module.parameters(recurse=recurse):
                        yield f"{name}.{subname}", subparam
                elif isinstance(module, torch.nn.Module):
                    for subname, subparam in module.named_parameters(recurse=recurse):
                        yield f"{name}.{subname}", subparam
