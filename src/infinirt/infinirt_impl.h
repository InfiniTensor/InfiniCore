...
#define INFINIRT_DEVICE_API(IMPL, COUNT)                                                                                            \
    ...
    infiniStatus_t getMemProp(infinirtMemProp_t *prop_ptr, infiniDevice_t device, int device_id) IMPL;                              \
    infiniStatus_t getMemGranularityMinimum(size_t *granularity, infinirtMemProp_t prop) IMPL;                                      \
    infiniStatus_t createPhysicalMem(infinirtPhyMem *phy_mem, size_t len, infinirtMemProp_t prop) IMPL;                             \
    infiniStatus_t createVirtualMemManager(infinirtVirtualMemManager *vm, infiniDevice_t device, size_t len, size_t min_addr) IMPL; \
    infiniStatus_t mapVirtualMem(void **mapped_ptr, infinirtVirtualMemManager *vm, size_t offset, infinirtPhyMem *phy_mem) IMPL;    \
    infiniStatus_t unmapVirtualMem(infinirtVirtualMemManager *vm, size_t offset) IMPL;
