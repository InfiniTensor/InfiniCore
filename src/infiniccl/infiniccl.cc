...
infiniStatus_t getDevice(infiniDevice_t *device_type, int *device_id) {
    const auto runtime_type = ::infini::rt::runtime_device_type();
    const auto translated_type = infinicore::bridge::infini::rt::translate_from(runtime_type);
    if (translated_type == INFINI_DEVICE_TYPE_COUNT) {
        return INFINI_STATUS_DEVICE_TYPE_NOT_SUPPORTED;
    }
    if (device_type != nullptr) {
        *device_type = translated_type;
    }
    if (device_id != nullptr) {
        return infinicore::bridge::infini::rt::translate(
            ::infini::rt::runtime::GetDevice(device_id));
    }
...
__INFINI_C infiniStatus_t infinicclGetUniqueId(
    ...
    infiniDevice_t device_type;
    CHECK_STATUS(infiniccl::getDevice(&device_type, nullptr));
