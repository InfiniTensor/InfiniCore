...
void w8a8_group_gemm_(Tensor out, const Tensor &input, const Tensor &weight, const Tensor &input_scale, const Tensor &weight_scale, const Tensor &tokens_per_experts, std::optional<Tensor> sorted_token_ids, std::optional<Tensor> bias, bool trans_weight, bool is_decode) {
    ...
    if (is_decode) {
        throw std::runtime_error("w8a8_group_gemm decode path is disabled: vllm_iluvatar cuinfer decode currently reports internal error");
    }
    ...
    if (out->device().getType() == Device::Type::ILUVATAR) {
        if (!adaptor::vllm_iluvatar::w8a8_group_gemm_available()) {
            throw std::runtime_error("w8a8_group_gemm requires vllm_iluvatar cuinfer extension");
        }
        ...
        adaptor::vllm_iluvatar::w8a8_group_gemm(o, in, w, is, ws, tpe, sti, bi, trans_weight, is_decode);
        return;
    }
#endif
    throw std::runtime_error("w8a8_group_gemm currently supports only Iluvatar builds with ATen and vllm_iluvatar");
}
} // namespace infinicore::op
