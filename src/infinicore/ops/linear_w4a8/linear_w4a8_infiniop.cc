...
namespace infinicore::op::linear_w4a8_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, LinearW4A8, 100);
...
void *plan(Tensor output,
    ...
    INFINIOP_CACHABLE_DESCRIPTOR_GET_OR_CREATE(
        Descriptor, descriptor, LinearW4A8, seed,
        output->desc(), input->desc(), packed_weight->desc(),
        weight_scale->desc(),
        bias.has_value() ? bias.value()->desc() : nullptr, alpha);
    INFINIOP_WORKSPACE_TENSOR(workspace, LinearW4A8, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopLinearW4A8(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(LinearW4A8, &plan, &run, &cleanup);
