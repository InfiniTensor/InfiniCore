...
namespace infinicore::op::log10_impl::cpu {
...
void calculate(Tensor output, Tensor input) {
    ...
    for (size_t idx = 0; idx < numel; ++idx) {
        ...
        // Compute log10 for the current element
        if (dtype == DataType::F32) {
            ...
            *output_ptr = std::log10(*input_ptr);
            ...
            *output_ptr = std::log10(*input_ptr);
            ...
            float output_f32 = std::log10(input_f32);
            *output_ptr = utils::cast<fp16_t>(output_f32);
        } else {
            throw std::runtime_error("Unsupported data type for log10 operation.");
        }
...
static bool registered = []() {
    Log10::dispatcher().registerDevice(Device::Type::CPU, &calculate);
    return true;
}();
