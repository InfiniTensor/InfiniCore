...
namespace infinicore::op::log1p_impl::cpu {
...
void calculate(Tensor output, Tensor input) {
    ...
    for (size_t idx = 0; idx < numel; ++idx) {
        ...
        // Compute log1p for the current element
        if (dtype == DataType::F32) {
            ...
            *output_ptr = std::log(*input_ptr + 1);
            ...
            *output_ptr = std::log(*input_ptr + 1);
            ...
            float output_f32 = std::log(input_f32 + 1);
            *output_ptr = utils::cast<fp16_t>(output_f32);
        } else {
            throw std::runtime_error("Unsupported data type for log1p operation.");
        }
...
static bool registered = []() {
    Log1p::dispatcher().registerDevice(Device::Type::CPU, &calculate);
    return true;
}();
