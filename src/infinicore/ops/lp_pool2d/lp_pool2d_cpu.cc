...
void calculate(Tensor output, Tensor input, float norm_type, tuple_size_2d kernel_size, tuple_size_2d stride, bool ceil_mode) {
    ...
    const size_t H_in = input_shapes[2];
    const size_t W_in = input_shapes[3];

    const size_t H_out = output_shapes[2];
    const size_t W_out = output_shapes[3];

    const size_t stride_N = input_strides[0];
    const size_t stride_C = input_strides[1];
    const size_t stride_H = input_strides[2];
    const size_t stride_W = input_strides[3];

    const size_t kernel_h = std::get<0>(kernel_size);
    const size_t kernel_w = std::get<1>(kernel_size);
    const size_t stride_h = std::get<0>(stride);
    const size_t stride_w = std::get<1>(stride);
                ...
                for (size_t ow = 0; ow < W_out; ++ow) {
                    ...
                    for (size_t ih = h_start; ih < h_end; ++ih) {
                        for (size_t iw = w_start; iw < w_end; ++iw) {
                            const size_t offset = n * stride_N + c * stride_C + ih * stride_H + iw * stride_W;
                    ...
                    // 处理 ceil_mode：用 replicate padding 放大到完整窗口
                    const size_t valid_h = h_end - h_start;
                    const size_t valid_w = w_end - w_start;
                    const size_t valid_cnt = valid_h * valid_w;
                    const size_t full_cnt = kernel_h * kernel_w;

                    double scale = 1.0;
                    if (ceil_mode && valid_cnt < full_cnt) {
                        // 为了匹配 torch 的行为：mean(valid) * kernel_size
                        scale = static_cast<double>(full_cnt) / static_cast<double>(valid_cnt);
                    }

                    const double norm = std::pow(sum_power * scale, 1.0 / norm_type);

                    const size_t out_offset = n * (C * H_out * W_out) + c * (H_out * W_out) + oh * W_out + ow;
                    if (dtype == DataType::F32) {
...
static bool registered = []() {
    Lp_Pool2d::dispatcher().registerDevice(Device::Type::CPU, &calculate);
    return true;
}();
