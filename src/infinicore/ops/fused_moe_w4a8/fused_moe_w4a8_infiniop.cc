...
namespace infinicore::op::fused_moe_w4a8_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, FusedMoeW4A8, 100);
...
void *plan(Tensor output,
    ...
    size_t seed = hash_combine(
        ...
        static_cast<int>(activation), weights_are_aiter_shuffled);
    INFINIOP_CACHABLE_DESCRIPTOR_GET_OR_CREATE(
        Descriptor, descriptor, FusedMoeW4A8, seed,
        output->desc(), input->desc(), selected_experts->desc(),
        routing_weights->desc(), w13_packed->desc(), w13_scale->desc(),
        w2_packed->desc(), w2_scale->desc(),
        static_cast<infiniopFusedMoeActivation_t>(activation),
        weights_are_aiter_shuffled);
    INFINIOP_WORKSPACE_TENSOR(workspace, FusedMoeW4A8, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopFusedMoeW4A8(
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(FusedMoeW4A8, &plan, &run, &cleanup);
