...
struct WorkspaceBuf { void *ptr = nullptr; size_t cap = 0; };
...
static aclDataType to_acl_dtype(DataType dtype) {
    switch (dtype) {
    case DataType::F16:  return ACL_FLOAT16;
    case DataType::BF16: return ACL_BF16;
    default:
        throw std::runtime_error(
            "[linear_allreduce/ascend] unsupported dtype: " +
            std::to_string(static_cast<int>(dtype)) +
            ". aclnnMatmulAllReduce only supports F16/BF16");
    }
}
    ...
    infinicclComm_t communicator)
{
    ...
    if (atype != DataType::F16 && atype != DataType::BF16) {
        throw std::runtime_error(
            "[linear_allreduce/ascend] unsupported activation dtype: " +
            std::to_string(static_cast<int>(atype)) +
            ". aclnnMatmulAllReduce only supports F16/BF16");
    }
    ...
    if (ret != 0) {
        if (bias_acl) aclDestroyTensor(bias_acl);
        aclDestroyTensor(x1_acl);
        aclDestroyTensor(x2_acl);
        aclDestroyTensor(out_acl);
        const char *err = aclGetRecentErrMsg();
        throw std::runtime_error(
            std::string("[linear_allreduce/ascend] GetWorkspaceSize failed: ") +
            std::to_string(ret) + ", msg: " + (err ? err : "(null)"));
    }
    ...
    if (ret != 0) {
        const char *err = aclGetRecentErrMsg();
        throw std::runtime_error(
            std::string("[linear_allreduce/ascend] execution failed: ") +
            std::to_string(ret) + ", msg: " + (err ? err : "(null)"));
    }
}
