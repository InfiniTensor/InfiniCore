...
void scaled_mm_w4a8_(Tensor out, const Tensor &a, const Tensor &b, const Tensor &a_scales, const Tensor &b_scales, std::optional<Tensor> bias, bool trans_weight) {
    ...
    if (out->device().getType() == Device::Type::ILUVATAR) {
        if (!adaptor::vllm_iluvatar::scaled_mm_w4a8_available()) {
            throw std::runtime_error("scaled_mm_w4a8 requires vllm_iluvatar cuinfer extension");
        }
        ...
        adaptor::vllm_iluvatar::scaled_mm_w4a8(o, aa, bb, as, bs, bi, trans_weight);
        return;
    }
#endif
    throw std::runtime_error("scaled_mm_w4a8 currently supports only Iluvatar builds with ATen and vllm_iluvatar");
}
} // namespace infinicore::op
