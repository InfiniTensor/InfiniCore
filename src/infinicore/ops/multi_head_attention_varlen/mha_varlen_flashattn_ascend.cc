...
void run(void *planned_meta) {
  ...
  aclIntArray *actual_seq_q_acl =
      host_vector_to_acl_int_array(actual_seq_q_vec);
  aclIntArray *actual_seq_k_acl =
      host_vector_to_acl_int_array(actual_seq_k_vec);
  ...
  if (workspace_size > 0) {
    ...
    if (alloc_ret != ACL_SUCCESS) {
      ...
      aclDestroyIntArray(actual_seq_k_acl);
      throw std::runtime_error(
          std::string("[mha_varlen/ascend] aclrtMalloc workspace failed: ") +
