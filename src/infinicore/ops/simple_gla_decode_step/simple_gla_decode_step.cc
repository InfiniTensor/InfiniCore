...
void simple_gla_decode_step_cpu_impl(Tensor &out, Tensor &state, const Tensor &q, const Tensor &k,
    ...
    INFINICORE_ASSERT(state->shape().size() == 4 && state->shape()[0] == B && state->shape()[1] == H && state->shape()[2] == D && state->shape()[3] == D);
...
void SimpleGlaDecodeStep::execute(Tensor &out, Tensor &state, const Tensor &q, const Tensor &k, const Tensor &v,
    ...
    if (func == nullptr) {
        throw std::runtime_error("simple_gla_decode_step: no implementation for device type " + std::to_string(static_cast<int>(device_type)));
    }
