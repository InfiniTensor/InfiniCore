...
namespace infinicore::op::paged_attention_v2_impl::infiniop {

INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, PagedAttentionV2, 100);

struct PlannedMeta {
    std::shared_ptr<Descriptor> descriptor;
    graph::GraphTensor workspace;
    graph::GraphTensor out;
    graph::GraphTensor exp_sums;
    graph::GraphTensor max_logits;
    graph::GraphTensor tmp_out;
...
void *plan(Tensor &out,          // [num_seqs, num_heads, head_size]
    ...
    INFINIOP_CACHABLE_DESCRIPTOR_GET_OR_CREATE(
        Descriptor, descriptor, PagedAttentionV2,
        seed,
    ...
    INFINIOP_WORKSPACE_TENSOR(workspace, PagedAttentionV2, descriptor);
...
INFINICORE_GRAPH_OP_REGISTER_ALLDEVICE(PagedAttentionV2, &plan, &run, &cleanup);
