...
Tensor py_baddbmm(Tensor input, Tensor batch1, Tensor batch2, float beta = 1.0f, float alpha = 1.0f) {
    if(beta != 1.0f ||  alpha != 1.0f) {
        std::optional<Tensor> beta_tensor = Tensor::from_blob((void*)&beta, {}, DataType::F32, input->device());
        std::optional<Tensor> alpha_tensor = Tensor::from_blob((void*)&alpha, {}, DataType::F32, input->device());
