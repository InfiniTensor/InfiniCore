...
Runtime *ContextImpl::getFirstNonCpuRuntime() {
    std::lock_guard<std::mutex> lock(runtime_table_mutex_);
    // Try to find the first non-CPU device runtime
    for (int i = int(Device::Type::COUNT) - 1; i > 0; i--) {
        if (!runtime_table_[i].empty() && runtime_table_[i][0] != nullptr) {
            return runtime_table_[i][0].get();
        }
    }
    return nullptr;
}
...
Runtime *ContextImpl::getRuntime(Device device) {
    ...
    if (runtime_table_[device_type].size() <= device_index ||
        runtime_table_[device_type][device_index] == nullptr) {
