...
void Runtime::memcpyD2H(void *dst, const void *src, size_t size) {
    ...
    if (get_device_status == INFINI_STATUS_SUCCESS) {
        ...
        if (infinirt_device_type != expected_infinirt_type ||
            infinirt_device_id != static_cast<int>(device_.getIndex())) {
            ...
            if (get_device_status == INFINI_STATUS_SUCCESS &&
                (infinirt_device_type != expected_infinirt_type ||
                 infinirt_device_id != static_cast<int>(device_.getIndex()))) {
                ...
                throw std::runtime_error(
                    ...
                    "Runtime device is " + device_.toString() +
                    " but infinirt reports device_type=" + std::to_string((int)infinirt_device_type) +
                    ", device_id=" + std::to_string(infinirt_device_id) +
                    ". This will cause incorrect routing in infinirtMemcpy.");
            } else {
