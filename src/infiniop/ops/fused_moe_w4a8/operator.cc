...
#include "nvidia/fused_moe_w4a8_nvidia.cuh"
#endif
#ifdef ENABLE_HYGON_API
#include "hygon/fused_moe_w4a8_hygon.cuh"
#endif

__INFINI_C infiniStatus_t infiniopCreateFusedMoeW4A8Descriptor(
...
#define CREATE(CASE, NAMESPACE)                                                               \
    case CASE:                                                                                \
        return op::fused_moe_w4a8::NAMESPACE::Descriptor::create(                             \
            handle, reinterpret_cast<op::fused_moe_w4a8::NAMESPACE::Descriptor **>(desc_ptr), \
...
__INFINI_C infiniStatus_t infiniopGetFusedMoeW4A8WorkspaceSize(
    infiniopFusedMoeW4A8Descriptor_t desc,
    size_t *size) {
#define GET(CASE, NAMESPACE)                                                         \
    case CASE:                                                                       \
        *size = reinterpret_cast<const op::fused_moe_w4a8::NAMESPACE::Descriptor *>( \
                    desc)                                                            \
                    ->workspaceSize();                                               \
...
__INFINI_C infiniStatus_t infiniopFusedMoeW4A8(
    infiniopFusedMoeW4A8Descriptor_t desc,
    void *workspace,
...
#define CALCULATE(CASE, NAMESPACE)                                                  \
    case CASE:                                                                      \
        return reinterpret_cast<const op::fused_moe_w4a8::NAMESPACE::Descriptor *>( \
                   desc)                                                            \
            ->calculate(workspace, workspace_size, output, input, selected_experts, \
                        routing_weights, w13_packed, w13_scale, w2_packed,          \
                        w2_scale, stream)
    switch (desc->device_type) {
#ifdef ENABLE_NVIDIA_API
...
__INFINI_C infiniStatus_t infiniopDestroyFusedMoeW4A8Descriptor(
    infiniopFusedMoeW4A8Descriptor_t desc) {
#define DESTROY(CASE, NAMESPACE)                                                    \
    case CASE:                                                                      \
        delete reinterpret_cast<const op::fused_moe_w4a8::NAMESPACE::Descriptor *>( \
            desc);                                                                  \
        return INFINI_STATUS_SUCCESS
    switch (desc->device_type) {
#ifdef ENABLE_NVIDIA_API
...
#endif
#ifdef ENABLE_HYGON_API
        DESTROY(INFINI_DEVICE_HYGON, hygon);
#endif
    default:
        return INFINI_STATUS_DEVICE_TYPE_NOT_SUPPORTED;
    }
