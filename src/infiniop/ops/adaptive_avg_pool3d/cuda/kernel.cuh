...
__device__ void adaptiveAvgPool3DKernel(T *y, const T *x, size_t N, size_t C,
  ...
  if (index < total) {
    ...
    size_t count =
        (x_d_end - x_d_start) * (x_h_end - x_h_start) * (x_w_end - x_w_start);
    for (size_t id = x_d_start; id < x_d_end; ++id) {
      for (size_t ih = x_h_start; ih < x_h_end; ++ih) {
        for (size_t iw = x_w_start; iw < x_w_end; ++iw) {
          size_t x_index = n * x_strides[0] + c * x_strides[1] +
                           id * x_strides[2] + ih * x_strides[3] +
                           iw * x_strides[4];
          sum += x[x_index];
        }
      }
    }
    size_t y_index = n * y_strides[0] + c * y_strides[1] + od * y_strides[2] +
                     oh * y_strides[3] + ow * y_strides[4];
