...
infiniStatus_t Descriptor::create(
    ...
    // 创建 Info 对象 (依赖修复后的 info.h 计算几何信息)
...
void calculate_cpu_impl(
    ...
    // 1. 定义计算精度类型
    // 如果 TData 是 double，使用 double 计算；否则（包括 float, fp16, bf16, int）提升到 float 计算
    using CalcType = typename std::conditional<std::is_same<TData, double>::value, double, float>::type;

    // 2. 转换 Alpha (使用 utils::cast 处理自定义类型)
    ...
    for (size_t o = 0; o < outer_size; ++o) {
        for (size_t i = 0; i < index_len; ++i) {
            ...
            for (size_t in = 0; in < inner_size; ++in) {
                // 【核心逻辑】
                // 1. 读取 Source 和 Output 旧值，使用 utils::cast 转为 CalcType (float/double)
