...
INFINIOP_MUSA_KERNEL causalSoftmax(
    ...
    Tdata max_0 = op::common_musa::reduce_op::max<BLOCK_SIZE, Tdata>(x, width);
    if (threadIdx.x == 0) {
        max_ = max_0;
    }
    __syncthreads();
    ...
    Tcompute sum_0 = op::common_musa::reduce_op::sum<BLOCK_SIZE, Tdata, Tcompute>(y, width);
    if (threadIdx.x == 0) {
        sum_ = sum_0;
    }
    __syncthreads();
