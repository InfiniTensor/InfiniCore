...
infiniStatus_t Descriptor::calculate(
    ...
    if (in_dtype == INFINI_DTYPE_I32) {
        if (out_dtype == INFINI_DTYPE_BOOL) {
            ...
            // Use homogeneous path; LogicalOrOp returns bool which is
            // implicitly converted to int32 (0/1).
            return _device_info->calculate<LogicalOrOp, int32_t>(_info, output, inputs, stream);
        }
    } else if (in_dtype == INFINI_DTYPE_U8) {
        if (out_dtype == INFINI_DTYPE_BOOL) {
            // Inputs uint8, output bool
            return _device_info->calculate<LogicalOrOp, bool, uint8_t, uint8_t>(_info, output, inputs, stream);
        } else if (out_dtype == INFINI_DTYPE_U8) {
            // Inplace(a/b): inputs and output are uint8
            return _device_info->calculate<LogicalOrOp, uint8_t>(_info, output, inputs, stream);
        }
    }
