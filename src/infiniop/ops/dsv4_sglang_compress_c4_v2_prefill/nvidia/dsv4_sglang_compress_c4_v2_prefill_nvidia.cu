#include "../../../handle.h"
#include "dsv4_sglang_compress_c4_v2_prefill_nvidia.cuh"
...
constexpr const char *kSymbol = "_Z38sglang_compress_c4_v2_prefill_bf16_512N3tvm3ffi10TensorViewES1_S1_S1_S1_S1_";
...
tvm::ffi::TensorView makeView(const void *data, const op::dsv4_sglang_compress_c4_v2_prefill::TensorInfo &info, int dev, DLTensor &t) {
...
#endif
namespace op::dsv4_sglang_compress_c4_v2_prefill::nvidia {
infiniStatus_t Descriptor::create(infiniopHandle_t handle, Descriptor **desc_ptr, infiniopTensorDescriptor_t kv_buffer_desc, infiniopTensorDescriptor_t kv_input_desc, infiniopTensorDescriptor_t kv_output_desc, infiniopTensorDescriptor_t ape_desc, infiniopTensorDescriptor_t plan_c_desc, infiniopTensorDescriptor_t plan_w_desc) {
    Info info;
    CHECK_STATUS(createInfo(&info, kv_buffer_desc, kv_input_desc, kv_output_desc, ape_desc, plan_c_desc, plan_w_desc));
    *desc_ptr = new Descriptor(info, 0, handle->device, handle->device_id);
    return INFINI_STATUS_SUCCESS;
}
infiniStatus_t Descriptor::calculate(void *, size_t, const void *kv_buffer, const void *kv_input, void *kv_output, const void *ape, const void *plan_c, const void *plan_w, void *) const {
#if defined(ENABLE_HYGON_API)
    auto fn = resolveFn();
    CHECK_OR_RETURN(fn != nullptr, INFINI_STATUS_INTERNAL_ERROR);
    int dev = this->device_id;
    DLTensor kv_buffer_t;
    auto kv_buffer_v = makeView(kv_buffer, _info.tensors[0], dev, kv_buffer_t);
    DLTensor kv_input_t;
    auto kv_input_v = makeView(kv_input, _info.tensors[1], dev, kv_input_t);
    ...
    fn(kv_buffer_v, kv_input_v, kv_output_v, ape_v, plan_c_v, plan_w_v);
    return INFINI_STATUS_SUCCESS;
#else
