...
infiniStatus_t launchKernel(const void *data, int64_t *tmp, int64_t *results,
  ...
  if (dtype == INFINI_DTYPE_F32) {
    ...
    //     <<<ceildiv(M, 512), 256, sizeof(size_t) * M * 2>>>(
    //         static_cast<const float *>(data), tmp, N, shapes, strides, ndim);
    int block_size = maxThreadsPerBlock * 2;
    int num_blocks = (N + block_size - 1) / block_size;
    // spdlog::debug("argwhere launchKernel num_blocks:{}, block_size:{}",
    ...
    spdlog::debug("argwhere launchKernel done");
...
infiniStatus_t Descriptor::create(infiniopHandle_t handle,
  ...
  size_t workspace_size =
      x_desc->ndim() * sizeof(size_t) * 2 +
      x_desc->ndim() * sizeof(int64_t) * x_desc->numel() * 2 +
      sizeof(int64_t) * opaque->internal->maxThreadsPerBlock();
...
infiniStatus_t Descriptor::calculate(void *workspace, size_t workspace_size,
  ...
  int64_t *result =
      reinterpret_cast<int64_t *>(tmp + _info.num_elements * ndim);
  int64_t *block_sum =
      reinterpret_cast<int64_t *>(result + _info.num_elements * ndim);
