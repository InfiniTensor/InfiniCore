...
void paged_attention_v2_launcher(
    ...
    const op::paged_attention_v2::PagedAttentionV2Info &info,
...
#define CALL_V2_LAUNCHER(T, CACHE_T, BLOCK_SIZE, KV_DTYPE, IS_BLOCK_SPARSE)     \
    paged_attention_v2_launcher<T, CACHE_T, BLOCK_SIZE, KV_DTYPE,               \
        ...
        blocksparse_head_sliding_step, _info, stream);
...
infiniStatus_t Descriptor::create(
    ...
    auto info_res = PagedAttentionV2Info::create(out_desc, query_desc, key_cache_desc, value_cache_desc, block_tables_desc,
...
infiniStatus_t Descriptor::calculate(
    ...
    if (query_dtype == INFINI_DTYPE_F32) {
        CALL_V2_LAUNCHER_BLOCK_SIZE(float, float, Fp8KVCacheDataType::kAuto);
    } else if (query_dtype == INFINI_DTYPE_F16) {
        CALL_V2_LAUNCHER_BLOCK_SIZE(uint16_t, uint16_t, Fp8KVCacheDataType::kAuto);
    } else if (query_dtype == INFINI_DTYPE_BF16) {
        CALL_V2_LAUNCHER_BLOCK_SIZE(__nv_bfloat16, __nv_bfloat16, Fp8KVCacheDataType::kAuto);
    } else {
