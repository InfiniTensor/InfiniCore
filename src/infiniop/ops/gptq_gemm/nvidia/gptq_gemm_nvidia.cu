...
INFINIOP_CUDA_KERNEL gemm_half_q_half_gptq_4bit_kernel(
...
template <bool first_block, int m_count>
INFINIOP_CUDA_KERNEL gemm_half_q_half_gptq_2bit_kernel(
    const half *__restrict__ a, const uint32_t *__restrict__ b_q_weight,
    const uint32_t *__restrict__ b_gptq_qzeros,
    const half *__restrict__ b_gptq_scales, half *__restrict__ c,
    const int size_m, const int size_n, const int size_k, const int groups,
...
template <bool first_block, int m_count>
INFINIOP_CUDA_KERNEL gemm_half_q_half_gptq_3bit_kernel(
    const half *__restrict__ a, const uint32_t *__restrict__ b_q_weight,
    const uint32_t *__restrict__ b_gptq_qzeros,
    const half *__restrict__ b_gptq_scales, half *__restrict__ c,
    const int size_m, const int size_n, const int size_k, const int groups,
...
template <bool first_block, int m_count>
INFINIOP_CUDA_KERNEL gemm_half_q_half_gptq_8bit_kernel(
    const half *__restrict__ a, const uint32_t *__restrict__ b_q_weight,
    const uint32_t *__restrict__ b_gptq_qzeros,
    const half *__restrict__ b_gptq_scales, half *__restrict__ c,
    const int size_m, const int size_n, const int size_k, const int groups,
...
#define SELECT_KERNEL(M_COUNT)                                       \
    if (m_count == M_COUNT) {                                        \
        if (bit == 2)                                                \
            return gemm_half_q_half_gptq_2bit_kernel<true, M_COUNT>; \
        if (bit == 3)                                                \
            return gemm_half_q_half_gptq_3bit_kernel<true, M_COUNT>; \
        if (bit == 4)                                                \
            return gemm_half_q_half_gptq_4bit_kernel<true, M_COUNT>; \
        if (bit == 8)                                                \
            return gemm_half_q_half_gptq_8bit_kernel<true, M_COUNT>; \
    }
...
INFINIOP_CUDA_KERNEL reconstruct_exllama_8bit_kernel(
...
INFINIOP_CUDA_KERNEL reconstruct_exllama_4bit_kernel(
    const uint32_t *__restrict__ b_q_weight, const int *__restrict__ b_q_perm,
    const uint32_t *__restrict__ b_gptq_qzeros,
    const half *__restrict__ b_gptq_scales, const int size_k, const int size_n,
    const int groups, half *__restrict__ b) {
...
INFINIOP_CUDA_KERNEL reconstruct_exllama_3bit_kernel(
    const uint32_t *__restrict__ b_q_weight, const int *__restrict__ b_q_perm,
    const uint32_t *__restrict__ b_gptq_qzeros,
    const half *__restrict__ b_gptq_scales, const int size_k, const int size_n,
    const int groups, half *__restrict__ b) {
...
INFINIOP_CUDA_KERNEL reconstruct_exllama_2bit_kernel(
    const uint32_t *__restrict__ b_q_weight, const int *__restrict__ b_q_perm,
    const uint32_t *__restrict__ b_gptq_qzeros,
    const half *__restrict__ b_gptq_scales, const int size_k, const int size_n,
    const int groups, half *__restrict__ b) {
...
INFINIOP_CUDA_KERNEL gemm_half_q_half_alt_4bit_kernel(
    ...
    while (k < h_end) {
        ...
        for (int tmp_k = 0; tmp_k < 4; tmp_k++) {
            ...
            half2 zero = __halves2half2(
                __hmul(scale_f,
                       __int2half_rn(-((zeros[g * zero_width + z_w] >> z_mod) & 0xF) - 1)),
                __hmul(scale_f2,
                       __int2half_rn(
...
#ifndef USE_ROCM
            // res2 = {};
            res2 = __halves2half2(__float2half(0.0f), __float2half(0.0f));
#else
...
INFINIOP_CUDA_KERNEL gemm_half_q_half_alt_8bit_kernel(
    const half2 *__restrict__ vec, const uint32_t *__restrict__ mat,
    half *__restrict__ mul, const half *__restrict__ scales,
    const uint32_t *__restrict__ zeros, const int *__restrict__ g_idx,
    int batch, int height, int width) {
...
INFINIOP_CUDA_KERNEL reconstruct_gptq_kernel(const uint32_t *__restrict__ w,
...
INFINIOP_CUDA_KERNEL reconstruct_gptq_3bit_kernel(
...
INFINIOP_CUDA_KERNEL shuffle_4bit_kernel(uint32_t *__restrict__ b_q_weight,
...
INFINIOP_CUDA_KERNEL shuffle_8bit_kernel(uint32_t *__restrict__ b_q_weight,
                                         const int size_k, const int size_n) {
    auto n = blockIdx.x * THREADS_X + threadIdx.x;
    if (n >= size_n) {
        return;
...
INFINIOP_CUDA_KERNEL shuffle_2bit_kernel(uint32_t *__restrict__ b_q_weight,
                                         const int size_k, const int size_n) {
    auto n = blockIdx.x * THREADS_X + threadIdx.x;
    if (n >= size_n) {
        return;
...
INFINIOP_CUDA_KERNEL shuffle_3bit_kernel(uint32_t *__restrict__ b_q_weight,
                                         const int size_k, const int size_n) {
    auto n = blockIdx.x * THREADS_X + threadIdx.x;
    if (n >= size_n) {
        return;
...
INFINIOP_CUDA_KERNEL make_sequential_4bit_kernel(const uint32_t *__restrict__ w,
...
INFINIOP_CUDA_KERNEL make_sequential_2bit_kernel(const uint32_t *__restrict__ w,
                                                 uint32_t *__restrict__ w_new,
                                                 const int *__restrict__ q_perm,
                                                 const int w_width) {
    const uint64_t *w2 = (uint64_t *)w;
...
INFINIOP_CUDA_KERNEL make_sequential_3bit_kernel(const uint32_t *__restrict__ w,
                                                 uint32_t *__restrict__ w_new,
                                                 const int *__restrict__ q_perm,
                                                 const int w_width) {
...
INFINIOP_CUDA_KERNEL make_sequential_8bit_kernel(const uint32_t *__restrict__ w,
                                                 uint32_t *__restrict__ w_new,
                                                 const int *__restrict__ q_perm,
                                                 const int w_width) {
    const uint64_t *w2 = (uint64_t *)w;
...
infiniStatus_t Descriptor::calculate(void *workspace,
    ...
    bool use_exllama = _info.use_exllama;
    int64_t quant_bit = _info.quant_bit;
