...
struct InfiniopBiAttentionDescriptor {
    ...
    infiniopSoftmaxDescriptor_t softmax_desc;
...
__C __export infiniStatus_t infiniopCreateBiAttentionDescriptor(infiniopHandle_t handle,
    ...
    infiniopSoftmaxDescriptor_t softmax_desc;
    CHECK_STATUS(infiniopCreateSoftmaxDescriptor(handle, &softmax_desc, qk_desc, qk_desc, 2));
    //      softmax workspace size
    size_t softmax_workspace_size;
    CHECK_STATUS(infiniopGetSoftmaxWorkspaceSize(softmax_desc, &softmax_workspace_size));
    ...
    *(InfiniopBiAttentionDescriptor **)desc_ptr = new InfiniopBiAttentionDescriptor{
...
__C __export infiniStatus_t infiniopGetBiAttentionWorkspaceSize(infiniopBiAttentionDescriptor_t desc, size_t *size) {
    *size = ((InfiniopBiAttentionDescriptor *)desc)->workspace_size;
    return INFINI_STATUS_SUCCESS;
}

__C __export infiniStatus_t infiniopBiAttention(infiniopBiAttentionDescriptor_t desc_,
                                              void *workspace_,
                                              ...
                                              void *v_cache,
                                              void *stream) {
    auto desc = (InfiniopBiAttentionDescriptor *)desc_;
    ...
    // softmax(qk)
    CHECK_STATUS(infiniopSoftmax(desc->softmax_desc,
                                       workspace, workspace_size,
...
__C __export infiniStatus_t infiniopDestroyBiAttentionDescriptor(infiniopBiAttentionDescriptor_t desc_) {
    auto desc = (InfiniopBiAttentionDescriptor *)desc_;
    if (desc->rearrange_desc_q) {
    ...
    CHECK_STATUS(infiniopDestroySoftmaxDescriptor(desc->softmax_desc));
    delete desc;
