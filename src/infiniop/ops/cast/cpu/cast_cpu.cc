...
infiniStatus_t Descriptor::create(
    ...
    // 因为不使用 device_impl，传 nullptr 和 workspace_size = 0
...
infiniStatus_t Descriptor::calculate(
    ...
    #define DISPATCH_CAST(SRC, DST) \
        return _device_info->calculate<CastOp, DST, SRC>(_info, output, inputs, stream);
    ...
    case INFINI_DTYPE_F16:
        switch (_input_dtype) {
        // 其它类型转换为 half 类型
        case INFINI_DTYPE_F16:     DISPATCH_CAST(fp16_t, fp16_t);
        case INFINI_DTYPE_F32:     DISPATCH_CAST(float, fp16_t);
        case INFINI_DTYPE_F64:     DISPATCH_CAST(double, fp16_t);
        case INFINI_DTYPE_I32:   DISPATCH_CAST(int32_t, fp16_t);
        case INFINI_DTYPE_I64:   DISPATCH_CAST(int64_t, fp16_t);
        case INFINI_DTYPE_U32:  DISPATCH_CAST(uint32_t, fp16_t);
        case INFINI_DTYPE_U64:  DISPATCH_CAST(uint64_t, fp16_t);
        default: break;
        }
        break;
    ...
    case INFINI_DTYPE_F32:
        switch (_input_dtype) {
        // 其它类型转换为 float 类型
        case INFINI_DTYPE_F16:     DISPATCH_CAST(fp16_t, float);
    ...
    case INFINI_DTYPE_F64:
        switch (_input_dtype) {
        case INFINI_DTYPE_F16:     DISPATCH_CAST(fp16_t, double);
