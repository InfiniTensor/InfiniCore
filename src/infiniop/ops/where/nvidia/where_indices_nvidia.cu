...
#include "where_indices_nvidia.h"
...
infiniStatus_t IndicesDescriptor::calculate(
    ...
    void **outputs, // outputs[i] 是第 i 维的索引张量
    const void *cond,
    void *stream,
    ...
    // 复制 shape 和 strides 到设备（用于 markTrueElements / collectIndices）
    size_t *d_shape;
    ptrdiff_t *d_strides;
    ...
    *num_true = static_cast<size_t>(num_true_val);
    ...
    // 阶段3: 收集每个维度的索引
    // 将 outputs 转换为 int64_t* 数组并复制到设备
