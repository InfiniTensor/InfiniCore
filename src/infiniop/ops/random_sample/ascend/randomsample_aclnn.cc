...
Descriptor::calculate(
    ...
    /////////////////
    // std::cout << "_info.n = " << _info.n << std::endl;
    // std::cout << "_info.dt_p = " << infiniSizeOf(_info.dt_p) << std::endl;
    // float *data = nullptr;
    // data = (float *)std::malloc(_info.n * infiniSizeOf(_info.dt_p));
    // aclrtMemcpy((void *)data, _info.n * infiniSizeOf(_info.dt_p), probs, _info.n * infiniSizeOf(_info.dt_p), ACL_MEMCPY_DEVICE_TO_HOST);
    // for (size_t i = 0; i < _info.n; i++) {
    //     std::cout << data[i] << " ";
    // }
    // std::cout << std::endl;
    //////////////////
    // std::cout << "topk_shape = " << topk_shape[0] << std::endl;
    ...
    //////////////////
    // float *dst = nullptr;
    // dst = (float *)std::malloc(topk_ * infiniSizeOf(_info.dt_p));
    // aclrtMemcpy((void *)dst, topk_ * infiniSizeOf(_info.dt_p), topk_val_addr, topk_ * infiniSizeOf(_info.dt_p), ACL_MEMCPY_DEVICE_TO_HOST);
    // for (int i = 0; i < topk_; i++) {
    //     std::cout << "topk_val[" << i << "] = " << dst[i] << std::endl;
    // }
    //////////////////
    ...
    // std::cout << "random_sample_kernel_launch success" << std::endl;
    // std::free(data);
    // std::free(dst);
