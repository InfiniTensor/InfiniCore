...
infiniStatus_t Descriptor::calculate(
    ...
    if (reduction == Reduction::NONE) {
        ...
        case INFINI_DTYPE_F16: {
            ...
            cuda::gaussian_nll_loss_kernel<half><<<num_blocks, BLOCK_SIZE, 0, cuda_stream>>>(
        ...
        case INFINI_DTYPE_BF16: {
            ...
            cuda::gaussian_nll_loss_kernel<cuda_bfloat16><<<num_blocks, BLOCK_SIZE, 0, cuda_stream>>>(
        ...
        case INFINI_DTYPE_F32: {
            ...
            cuda::gaussian_nll_loss_kernel<float><<<num_blocks, BLOCK_SIZE, 0, cuda_stream>>>(
        ...
        case INFINI_DTYPE_F64: {
            double eps_val = eps;
            cuda::gaussian_nll_loss_kernel<double><<<num_blocks, BLOCK_SIZE, 0, cuda_stream>>>(
        ...
        case INFINI_DTYPE_F32: {
            ...
            cuda::gaussian_nll_loss_reduce_kernel<float, float><<<num_blocks, BLOCK_SIZE, 0, cuda_stream>>>(
                reinterpret_cast<float *>(y),
                reinterpret_cast<const float *>(input),
                reinterpret_cast<const float *>(target),
                reinterpret_cast<const float *>(var),
                input_size, eps_val, full);
            // TODO: Scale by 1/n for MEAN
            break;
        }
        case INFINI_DTYPE_F64: {
            double eps_val = eps;
            CHECK_CUDA(cudaMemsetAsync(y, 0, sizeof(double), cuda_stream));
            cuda::gaussian_nll_loss_reduce_kernel<double, double><<<num_blocks, BLOCK_SIZE, 0, cuda_stream>>>(
                reinterpret_cast<double *>(y),
                reinterpret_cast<const double *>(input),
                reinterpret_cast<const double *>(target),
                reinterpret_cast<const double *>(var),
