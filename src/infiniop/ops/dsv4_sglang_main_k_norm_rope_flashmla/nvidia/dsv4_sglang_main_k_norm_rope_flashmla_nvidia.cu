#include "../../../handle.h"
#include "dsv4_sglang_main_k_norm_rope_flashmla_nvidia.cuh"
...
namespace {
using fn_t = void (*)(tvm::ffi::TensorView, tvm::ffi::TensorView, tvm::ffi::TensorView, tvm::ffi::TensorView, tvm::ffi::TensorView, tvm::ffi::TensorView, double);
constexpr const char *kSymbol = "_Z51sglang_main_k_norm_rope_flashmla_bf16_512_64_page64N3tvm3ffi10TensorViewES1_S1_S1_S1_S1_d";
...
infiniStatus_t Descriptor::create(infiniopHandle_t handle, Descriptor **desc_ptr, infiniopTensorDescriptor_t kv_desc, infiniopTensorDescriptor_t weight_desc, infiniopTensorDescriptor_t freqs_desc, infiniopTensorDescriptor_t positions_desc, infiniopTensorDescriptor_t out_loc_desc, infiniopTensorDescriptor_t cache_desc, double eps) {
    Info info;
    CHECK_STATUS(createInfo(&info, kv_desc, weight_desc, freqs_desc, positions_desc, out_loc_desc, cache_desc, eps));
    *desc_ptr = new Descriptor(info, 0, handle->device, handle->device_id);
    return INFINI_STATUS_SUCCESS;
}
infiniStatus_t Descriptor::calculate(void *, size_t, void *kv, const void *weight, const void *freqs, const void *positions, const void *out_loc, void *cache, void *) const {
#if defined(ENABLE_HYGON_API)
    auto fn = resolveFn();
    ...
    DLTensor kv_t, w_t, f_t, p_t, loc_t, cache_t;
    auto kv_v = makeView(kv, DLDataType{kDLBfloat, 16, 1}, kv_shape, kv_strides, dev, kv_t);
    auto w_v = makeView(weight, DLDataType{kDLBfloat, 16, 1}, weight_shape, weight_strides, dev, w_t);
    auto f_v = makeView(freqs, DLDataType{kDLFloat, 32, 1}, freq_shape, freq_strides, dev, f_t);
    auto p_v = makeView(positions, DLDataType{kDLInt, 32, 1}, pos_shape, pos_strides, dev, p_t);
    auto loc_v = makeView(out_loc, DLDataType{kDLInt, 32, 1}, pos_shape, pos_strides, dev, loc_t);
    auto cache_v = makeView(cache, DLDataType{kDLUInt, 8, 1}, cache_shape, cache_strides, dev, cache_t);
    fn(kv_v, w_v, f_v, p_v, loc_v, cache_v, i.eps);
    return INFINI_STATUS_SUCCESS;
#else
