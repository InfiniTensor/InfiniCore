...
template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE,
          int NUM_THREADS, Fp8KVCacheDataType KV_DTYPE,
...
__device__ void paged_attention_kernel(
    ...
    constexpr int NUM_THREAD_GROUPS = NUM_THREADS / THREAD_GROUP_SIZE; // Note: This assumes THREAD_GROUP_SIZE
                                                                       // divides NUM_THREADS
...
#pragma unroll
            for (int j = 0; j < NUM_VECS_PER_THREAD; j++) {
                const cache_t *k_ptr = k_cache + physical_block_number * kv_block_stride + kv_head_idx * kv_head_stride + physical_block_offset * x;
...
#pragma unroll
    ...
    for (int block_idx = start_block_idx + warp_idx; block_idx < end_block_idx;
        ...
        if constexpr (IS_BLOCK_SPARSE) {
            ...
            if (!((v_bs_block_id + bs_block_offset) % blocksparse_vert_stride == 0) && !((v_bs_block_id > q_bs_block_id - blocksparse_local_blocks))) {
                continue;
            }
        }
        const int64_t physical_block_number = static_cast<int64_t>(block_table[block_idx]);
...
__global__ void paged_attention_v1_kernel(
    ...
    paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,
        ...
        /* exp_sums */ nullptr, /* max_logits */ nullptr, out, q, k_cache, v_cache, num_kv_heads, scale,
