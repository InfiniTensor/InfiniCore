...
__global__ void permute_cols_kernel(
    ...
    int num_moe_blocks = device::div_ceil(num_tokens_past_padded, moe_block_size);
...
int get_scales_cache_size(
    ...
    if (group_size == -1) {
        ...
        tb_groups = device::div_ceil(tb_k, 32); // Worst case is 32 group size
    } else {
        tb_groups = device::div_ceil(tb_k, group_size);
    }
...
void marlin_mm(
    ...
    bool is_zp_float) {
    int thread_m_blocks = device::div_ceil(moe_block_size, 16);
