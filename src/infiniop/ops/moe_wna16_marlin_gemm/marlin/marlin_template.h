...
__global__ void Marlin(
    ...
    constexpr bool dequant_skip_flop = w_type == host::kFE4M3fn || w_type == host::kFE2M1f && s_type == host::kFE4M3fn || has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value || has_zp && !is_zp_float && !(w_type == host::kU8);
    ...
    const int zp_expert_stride = is_zp_float ? prob_n * prob_k / group_size / 8 : prob_n * prob_k / group_size / (pack_factor * 4);
    ...
    int iters = device::div_ceil(k_tiles * n_tiles * parallel, gridDim.x);
    ...
    if constexpr (!has_act_order && group_blocks != -1) {
        if (group_blocks >= thread_k_blocks) {
            ...
            iters = (group_blocks / thread_k_blocks) * device::div_ceil(iters, (group_blocks / thread_k_blocks));
        }
    }
    ...
    auto read_moe_block_data = [&](int block_id) {
        ...
        if (threadIdx.x >= threads - 32) {
            constexpr int size_per_thread = device::div_ceil(moe_block_size, 32);
...
#pragma unroll
    ...
    auto init_slice = [&](bool first_init = false) {
        ...
        slice_count = 1;
        slice_idx = 0;
        int col_first = iters * device::div_ceil(k_tiles * slice_col_par, iters);
        if (col_first <= k_tiles * (slice_col_par + 1)) {
            int col_off = col_first - k_tiles * slice_col_par;
            slice_count = device::div_ceil(k_tiles - col_off, iters);
            if (col_off > 0) {
                slice_count++;
            }
        ...
        if (first_init && use_atomic_add && slice_count > 1 && slice_idx == 0) {
            ...
            int m_per_thread = device::div_ceil(block_num_valid_tokens, threads / threads_per_m);
    ...
    constexpr int a_sh_wr_iters = device::div_ceil(a_sh_stage, a_sh_wr_delta);
    ...
    constexpr int s_tb_groups = !has_act_order && group_blocks != -1 && group_blocks < thread_k_blocks ? thread_k_blocks / group_blocks : 1;
...
#pragma unroll
            ...
            if constexpr (has_act_order) {
                ...
                if constexpr (group_blocks != -1) {
                    ...
                    if (pipe % device::div_ceil(group_blocks, thread_k_blocks) == 0) {
                        if (s_sh_wr_pred) {
...
#pragma nv_diagnostic pop
    ...
    auto matmul = [&](int k) {
        ...
        if constexpr (
            (s_type == host::kFE4M3fn || s_type == host::kFE8M0fnu) && !(std::is_same<scalar_t2, half2>::value && s_type == host::kFE8M0fnu)) {
...
#pragma unroll
        for (int i = 0; i < device::div_ceil(16 * thread_m_blocks, threads / (2 * thread_n_blocks)); i++) {
