...
INFINIOP_CUDA_KERNEL perTensorAbsmaxFp8(
    ...
    int num_elements) {
    perTensorAbsmaxFp8Kernel<Tdata, BLOCK_SIZE>(x_scale, x,
...
template <typename Tdata, unsigned int BLOCK_SIZE>
INFINIOP_CUDA_KERNEL perTensorQuantFp8Sym(
    ...
    int num_elements) {
    perTensorQuantFp8SymKernel<Tdata, BLOCK_SIZE>(x_packed, x_scale, x,
                                                  batch_size, channel, hidden_dim, width,
                                                  strides_0, strides_1, strides_2, strides_3,
...
infiniStatus_t Descriptor::create(
    ...
    auto info = PerTensorQuantFp8Info::createPerTensorQuantFp8Info(x_packed_desc, x_scale_desc, x_desc);
    CHECK_RESULT(info);
...
infiniStatus_t per_tensor_quant_fp8Kernel(const PerTensorQuantFp8Info &info, unsigned char *x_packed, float *x_scale, const Tdata *x, const bool is_static, cudaStream_t stream) {
    ...
    if (!is_static) {
        perTensorAbsmaxFp8<Tdata, BLOCK_SIZE>
            <<<num_blocks, BLOCK_SIZE, 0, stream>>>(x_scale, x,
                                                    batch_size, channel, hidden_dim, width,
                                                    strides_0, strides_1, strides_2, strides_3,
                                                    num_elements);
    }
    perTensorQuantFp8Sym<Tdata, BLOCK_SIZE>
...
#define QUANT(BLOCK_SIZE, TDATA) \
    per_tensor_quant_fp8Kernel<BLOCK_SIZE, TDATA>(_info, (unsigned char *)x_packed, (float *)x_scale, (const TDATA *)x, is_static, stream)
