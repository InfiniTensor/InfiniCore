...
struct DeepseekV4HashTopkRouterInfo {
    size_t num_tokens;
    size_t hidden_size;
    size_t num_experts;
    size_t topk;
    ...
    static utils::Result<DeepseekV4HashTopkRouterInfo> create(
        infiniopTensorDescriptor_t topk_weights_desc,
        infiniopTensorDescriptor_t topk_indices_desc,
        ...
        auto dtype = hidden_states_desc->dtype();
        if (dtype != INFINI_DTYPE_F16 && dtype != INFINI_DTYPE_BF16 && dtype != INFINI_DTYPE_F32) {
            return INFINI_STATUS_BAD_TENSOR_DTYPE;
        }
        ...
        if (hidden_states_desc->ndim() != 2 || weight_desc->ndim() != 2
            || topk_weights_desc->ndim() != 2 || topk_indices_desc->ndim() != 2
            || tid2eid_desc->ndim() != 2) {
            return INFINI_STATUS_BAD_TENSOR_SHAPE;
        }
        if (input_ids_desc->numel() != hidden_states_desc->shape()[0]) {
            return INFINI_STATUS_BAD_TENSOR_SHAPE;
        }
        const size_t num_tokens = hidden_states_desc->shape()[0];
        const size_t hidden_size = hidden_states_desc->shape()[1];
        const size_t num_experts = weight_desc->shape()[0];
        const size_t topk = topk_weights_desc->shape()[1];
        if (weight_desc->shape()[1] != hidden_size || topk == 0 || topk > num_experts
            || topk_weights_desc->shape()[0] != num_tokens
            || topk_indices_desc->shape()[0] != num_tokens
            || topk_indices_desc->shape()[1] != topk
            || tid2eid_desc->shape()[1] != topk) {
            return INFINI_STATUS_BAD_TENSOR_SHAPE;
        }
        if (hidden_states_desc->strides()[1] != 1 || weight_desc->strides()[1] != 1
            || topk_weights_desc->strides()[1] != 1 || topk_indices_desc->strides()[1] != 1
            || input_ids_desc->strides()[input_ids_desc->ndim() - 1] != 1
            || tid2eid_desc->strides()[1] != 1) {
            return INFINI_STATUS_BAD_TENSOR_STRIDES;
        }
        return utils::Result<DeepseekV4HashTopkRouterInfo>(DeepseekV4HashTopkRouterInfo{
            num_tokens,
            hidden_size,
            num_experts,
            topk,
