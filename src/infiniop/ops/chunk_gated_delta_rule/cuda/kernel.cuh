...
__device__ void chunkGatedDeltaRuleKernel(
    ...
    for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
        ...
        for (size_t i = thread_idx; i < chunk_size; i += BLOCK_THREADS) {
            ...
            for (size_t d = 0; d < Dk; ++d) k_s[i * Dk + d] *= beta_val;
            for (size_t d = 0; d < Dv; ++d) v_s[i * Dv + d] *= beta_val;
        ...
        for (size_t i = thread_idx; i < chunk_size * chunk_size; i += BLOCK_THREADS) {
            ...
            if (col <= row) {
                // Here k_s already contains k_beta. And the original k is needed for k^T.
                // We need to reload original k or store k_beta in a separate buffer.
                // For now, let's assume k_s still holds k and apply beta on the fly.
                // Reverting to apply beta later.
