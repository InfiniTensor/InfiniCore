...
public:
    ...
    create(infiniopTensorDescriptor_t out_desc,
        ...
        if (out_desc->dtype() != dtype || final_state_desc->dtype() != dtype || k_desc->dtype() != dtype || v_desc->dtype() != dtype || g_desc->dtype() != dtype || beta_desc->dtype() != dtype) {
            return INFINI_STATUS_BAD_TENSOR_DTYPE;
        }

        // Check tensor dimensions
        if (q_desc->ndim() != 4 || k_desc->ndim() != 4 || v_desc->ndim() != 4 || g_desc->ndim() != 3 || beta_desc->ndim() != 3 || out_desc->ndim() != 4 || final_state_desc->ndim() != 4) {
